...
from auth_app.schemas import User, UserCreate, ClientResponse
from auth_app.managers import UserManager, UserRole, RoleManager
import logging
import pyotp
...
from auth_app.utils import get_db_connection_batch_process, validate_token, send_password_reset_email
from auth_app.core import websocket_manager
from auth_app.services import get_auth_service
...
async def login(backgroudtasks : BackgroundTasks,form_data:OAuth2PasswordRequestForm = Depends(), 
    ...
    try:
        ...
        '''Performing a separate user existence check is redundant, as we need to query the users table anyway to 
        verify the user and retrieve the stored password. A single database query to handle both checks is more efficient.'''
...
async def logout_me(request:Request,
    ...
    try:
        ...
        if not isinstance(err, HTTPException):
            ...
            #Websocket broadcast
            await websocket_manager.broadcast(f"{datetime.now()} : User: {current_user [0]}, Result: {str(err)}")

            raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Internal server error')
        else:
            #Websocket broadcast
            await websocket_manager.broadcast(f"{datetime.now()} : User: {current_user [0]}, Result: {err.detail}")
